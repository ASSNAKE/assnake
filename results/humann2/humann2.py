rule humann2:
    input:
        ...
        mp2 = 'datasets/{df}/taxa/{preproc}/mp2__def/{sample}/{sample}.mp2'
    output:
        gf = 'datasets/{df}/humann2/{db_nucl}__{db_protein}/{sample}/{preproc}/{sample}_genefamilies.tsv',
        pc = 'datasets/{df}/humann2/{db_nucl}__{db_protein}/{sample}/{preproc}/{sample}_pathcoverage.tsv',
        pa = 'datasets/{df}/humann2/{db_nucl}__{db_protein}/{sample}/{preproc}/{sample}_pathabundance.tsv'
    params:
        wd =     'datasets/{df}/humann2/{db_nucl}__{db_protein}/{sample}/{preproc}/',
        merged = 'datasets/{df}/humann2/{db_nucl}__{db_protein}/{sample}/{preproc}/{sample}.fastq.gz'
    threads: 48
...
rule humann2_custom_kegg:
    input:
        ...
        mp2 = 'datasets/{df}/taxa/{preproc}/mp2__def/{sample}/{sample}.mp2'
    output:
        gf = 'datasets/{df}/humann2/KEGG/{sample}/{preproc}/{sample}_genefamilies.tsv',
        pc = 'datasets/{df}/humann2/KEGG/{sample}/{preproc}/{sample}_pathcoverage.tsv',
        pa = 'datasets/{df}/humann2/KEGG/{sample}/{preproc}/{sample}_pathabundance.tsv'
    params:
        wd =     'datasets/{df}/humann2/KEGG/{sample}/{preproc}/',
        merged = 'datasets/{df}/humann2/KEGG/{sample}/{preproc}/{sample}.fastq.gz'
    threads: 12
    conda: "../../envs/humann2.yml"
    shell: ('''cat {input.r1} {input.r2} > {params.merged} \n 
               humann2 --taxonomic-profile {input.mp2} \
...
rule humann2_regroup:
    input:
        gf = 'datasets/{df}/humann2/{db_nucl}__{db_protein}/{sample}/{preproc}/{sample}_genefamilies.tsv',
    output:
        gf = 'datasets/{df}/humann2/{db_nucl}__{db_protein}/{sample}/{preproc}/{sample}__{groups}.tsv',
    conda: "../../envs/humann2.yml"
...
rule humann2_norm:
    input:
        gf = 'datasets/{df}/humann2/{db_nucl}__{db_protein}/{sample}/{preproc}/{sample}_{groups}.tsv',
    output:
        norm = 'datasets/{df}/humann2/{db_nucl}__{db_protein}/{sample}/{preproc}/{sample}_{groups}__norm.tsv'
    conda: "../../envs/humann2.yml"
