...
from assnake.api.loaders import InputError
...
@click.pass_obj
def df_create(config, data_type, df_name, data_storage_folder, full_path_to_df,first_preprocessing_name, description, quietly, test_data):
	...
	df_info = {'df': df, 'fs_prefix': fs_prefix, 'description': {}, 'data_type': data_type, 
			   'full_path':full_df_path, 'dataset_type':None, 'processes':{}}
		...
		if os.path.isfile(df_info_loc):
			with open(df_info_loc, 'r') as stream:
				try:
					...
					pass
		if df_info is not {}:
			if 'processes' in df_info:
				click.echo('Processes:')
				for process_name, process_files in df_info['processes'].items():
...
def df_import_reads(config, reads_dir, dataset, rename_method, target, sample_set, sample_list, copy):
	...
	if len(samples_in_run) > 0:
		...
		df_info = assnake.Dataset(dataset)
		df_update_info(df_name=dataset, dataset_type=df_info.dataset_type)
...
#								   UNPDATE-INFO
# ---------------------------------------------------------------------------------------
def validate_df_prefix_existence(fs_prefix, allow_not_empty_full_path=True):
	"""
	Ensures that can write to full_df_path and it is not empty
	"""
	if not os.path.isabs(fs_prefix):
		click.secho('fs_prefix is not absolute!', fg='red') 
		exit()
...
def validate_full_path_existence(full_df_path, allow_not_empty_full_path=True):
	...
	Ensures that can write to full_df_path and it is not empty
	"""
	if not os.path.isabs(full_df_path):
		click.secho('Full path is not absolute!', fg='red') 
		exit()

	if os.path.isdir(full_df_path):
		if not os.listdir(full_df_path) and os.access(full_df_path, os.W_OK):
			click.echo('Warning! ' + full_df_path + ' exists, but is empty.')
			return full_df_path
		elif os.listdir(full_df_path):
...
@click.command(name='update-info', help = 'Update df_info.yml file.')
@click.option('--df-name', '-d', help='Name of the dataset.', required = True)
@click.option('--data-type', '-t', 
			help='Type of NGS data. Illumina Metagenomic WGS and 16s are supported.', 
			required = False,
			type=click.Choice(['METAGENOMIC_16s', 'METAGENOMIC_WGS', 'VIROME', 'RNA_SEQ'], case_sensitive=False))
@click.option('--dataset-type', '-r', help='Type of raw reads: either single-end or paired-end.', 
				type=click.Choice(['single-end', 'paired-end'], case_sensitive=False), required = False)
@click.option('--description', '-ds', help='Dataset description.', required = False)
@click.option('--prefix', '-pp', help='Prefix path to the dataset. If given, the full path will be changed automatically. If both given, nothing will happen automatically.', required = False)
@click.option('--full-path', '-fp', help='Full path to the dataset. If given, the prefix will be changed automatically. If both given, nothing will happen automatically.', required = False)
@click.option('--processes', '-p', help='Final files of all processes which were run by user with the assnake.', required = False)
@click.option('--clean_paths', '-c', help='Delete all processes paths that do not exist anymore.', required = False, is_flag=True)
...
def df_update_info(config=None, df_name=None, data_type=None, dataset_type=None, description=None, prefix=None, full_path=None, processes=None, clean_paths=False):
	...
	for name, new_info in zip(['fs_prefix', 'description', 'data_type', 
		...
		if new_info is not None:
			...
			was_changed.append(name)

	prefix_df_name_validation = True
	full_path_validation = True
	if ('fs_prefix' in was_changed): 
		prefix_df_name_validation = validate_df_prefix_existence(df_info['fs_prefix'])
		if ('full_path' not in was_changed) and (prefix_df_name_validation is not None):
			df_info['full_path'] = '%s/%s'%(df_info['fs_prefix'], df_info['df'])
			full_path_validation = validate_full_path_existence(df_info['full_path'])
		
	if ('full_path' in was_changed):		
		full_path_validation = validate_full_path_existence(df_info['full_path'])
		if ('fs_prefix' not in was_changed) and (full_path_validation is not None):
			df_info['prefix'] = os.dirname(df_info['full_path'])
			prefix_df_name_validation = validate_df_prefix_existence(df_info['fs_prefix'])
	...
	if clean_paths:
		if 'processes' in df_info:
			names_to_delete = []
			for process_name, process_files in df_info['processes'].items():
				process_files = [process_file for process_file in process_files 
				                 if os.path.exists('%s/%s'%(df_info['full_path'], process_file))]
				if len(process_files) == 0:
					names_to_delete.append(process_name)
				else:
					df_info['processes'][process_name] = process_files
			for process_name in names_to_delete:
				del df_info['processes'][process_name]
