...
if os.stat(sys.argv[1]).st_size == 0:
	...
	with open(sys.argv[1]) as HitDistribution:
		for line in HitDistribution:
			...
			contig_hit_dict[(contig)] = (contigHits)
			contigs_with_hits = contigs_with_hits + 1
	ReadsPerContigList = list()
	contigsList = list()
	for k,v in contig_hit_dict.iteritems():
	...
	for i in range(1,21):
		if int(i) in contigsList:
			...
			contig_hit_dict[EmptyContig] = EmptyContigValue
	ReadsPerContigList = contig_hit_dict.values()
	...
	Taxid = (str(sys.argv[3]).strip())
	PercentChroms = (float(contigs_with_hits) / float(NumberOfContigs) * 100)
